...
class CommandsComponent(Component):
    ...
    def __init__(self, install_dir: Optional[Path] = None):
        ...
        super().__init__(install_dir, Path("commands"))
    ...
    def uninstall(self) -> bool:
        ...
        try:
            ...
            # Remove command files from qwen subdirectory
            commands_dir = self.install_dir / "commands" / "qwen"
            removed_count = 0
            ...
            # Remove qwen subdirectory if empty
            try:
                if commands_dir.exists():
                    remaining_files = list(commands_dir.iterdir())
                    if not remaining_files:
                        commands_dir.rmdir()
                        self.logger.debug("Removed empty qwen commands directory")
    ...
    def validate_installation(self) -> Tuple[bool, List[str]]:
        ...
        # Check if qwen commands directory exists
        commands_dir = self.install_dir / "commands" / "qwen"
        if not commands_dir.exists():
            errors.append("qwen commands directory not found")
            return False, errors
    ...
    def get_installation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "install_directory": str(self.install_dir / "commands" / "qwen"),
            "dependencies": self.get_dependencies()
        }
    
    def _migrate_existing_commands(self) -> None:
        """Migrate existing commands from old location to new qwen subdirectory"""
        try:
            old_commands_dir = self.install_dir / "commands"
            new_commands_dir = self.install_dir / "commands" / "qwen"
            ...
            if commands_to_migrate:
                self.logger.info(f"Found {len(commands_to_migrate)} existing commands to migrate to qwen/ subdirectory")
                
                # Ensure new directory exists
                if not self.file_manager.ensure_directory(new_commands_dir):
                    self.logger.error(f"Could not create qwen commands directory: {new_commands_dir}")
                    return
                ...
                for filename in commands_to_migrate:
                    ...
                    try:
                        ...
                        if self.file_manager.copy_file(old_file_path, new_file_path):
                            ...
                            if self.file_manager.remove_file(old_file_path):
                                migrated_count += 1
                                self.logger.debug(f"Migrated {filename} to qwen/ subdirectory")
                            else:
                                self.logger.warning(f"Could not remove old {filename}")
                        else:
                            self.logger.warning(f"Could not copy {filename} to qwen/ subdirectory")
                    except Exception as e:
                        self.logger.warning(f"Error migrating {filename}: {e}")
                
                if migrated_count > 0:
                    self.logger.success(f"Successfully migrated {migrated_count} commands to /qwen: namespace")
                    self.logger.info("Commands are now available as /qwen:analyze, /qwen:build, etc.")
        ...
        try:
            ...
            for md_file in commands_dir.glob("*.md"):
                try:
                    ...
                    prompt = f"""SuperGemini Framework Command: /qwen:{command_name}
