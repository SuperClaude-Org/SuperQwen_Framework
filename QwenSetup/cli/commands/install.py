"""
Qwen Installation Operation Module
...
def register_parser(subparsers, global_parser=None) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        "install",
        help="Install Qwen framework components",
        description="Install Qwen Framework with various options and profiles",
        epilog="""
Examples:
  Qwen install                          # Interactive installation
  Qwen install --dry-run                # Dry-run mode  
  Qwen install --components core mcp    # Specific components
  Qwen install --verbose --force        # Verbose with force mode
        """,
...
def run_install(args: argparse.Namespace) -> bool:
    """
    Execute Qwen installation operation.
    ...
    # Display header
    display_header("Qwen Installation v4.0.4", 
                  "Installing Qwen framework components")
...
def install_with_profile(args: argparse.Namespace) -> bool:
    """
    Install Qwen with a predefined profile.
...
def execute_install_workflow(args: argparse.Namespace, components: List[str], registry: ComponentRegistry) -> bool:
    ...
    try:
        ...
        if success:
            ...
            display_info("3. Use Qwen commands and features in Gemini CLI")
